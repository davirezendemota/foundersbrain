...
2. **Conventional Commits (in English only)**
   ...
   * Stage and commit with Git from the repository root (no wrapper scripts). Examples:
     ...
     Without scope: `git commit -m "fix: resolve null pointer in parser"`. For a multi-line body, use a heredoc so quoting stays safe:
     
     ```bash
     git commit -m "$(cat <<'EOF'
docs: update readme

Optional body line after blank line.
EOF
)"
     ```
...
3. **Branch: Always Use `main`**
   ...
   * If the current branch is **not** `main`, switch to it:
     ...
     git checkout main
     ```
   * Commit directly to `main`. Do **not** create a new branch. This command is for pushing changes straight to `main`.
...
6. **Optional: Discord summary after push (explicit channel only)**
   ...
   * **Never** pick a Discord channel automatically. Follow `ai/rules/discord-agent-workflow.md`.
   * After a successful `git push origin main`, only post to Discord if the user explicitly asked for a Discord notification **and** supplied a **channel ID** for that message. Use:

     ```bash
     scripts/discord/send-message.sh CHANNEL_ID "Short summary (under 2000 chars)."
     ```
